...
export default function App() {
  ...
  const pushHistory = (currentColors: PaletteColor[]) => {
    setHistory(prev => [currentColors, ...prev].slice(0, 15));
  };
  ...
  // URL Palette Share Check on Initial Mount
  useEffect(() => {
  ...
  const copyAllColors = async () => {
    const text = rawColors.join(", ");
    await navigator.clipboard.writeText(text);
    flash(t.copiedAll);
  };

  const sharePalette = async () => {
    const cleanHexes = rawColors.map(c => c.replace("#", "")).join(",");
    const url = `${window.location.origin}${window.location.pathname}?colors=${cleanHexes}`;
  ...
  const savePalette = () => {
    const defaultName = `${t.savedDefaultName || "TAYFORA"} · ${saved.length + 1}`;
  ...
  const startRenameSaved = (item: SavedPalette) => {
    setEditingSavedId(item.id);
    setEditingName(item.name);
  };

  const saveRenameSaved = (id: string) => {
    const next = saved.map(item => (item.id === id ? { ...item, name: editingName || item.name } : item));
    setSaved(next);
    setSavedPalettes(next);
  ...
  const downloadExport = () => {
    ...
    a.href = url;
    a.download = `tayfora-palette.${ext}`;
    a.click();
  ...
  return (
    <div className="app-shell" dir={dir}>
      ...
      <header className="site-header">
        <a className="brand" href="#top" onClick={e => { e.preventDefault(); openPage("lab"); }} aria-label={t.brandName}>
        ...
        <nav aria-label="Primary">
          {PAGES.map(item => (
            <button key={item} className={page === item ? "active" : ""} onClick={() => openPage(item)}>
              {navLabels[item]}
              {item === "saved" && saved.length > 0 && <sup>{saved.length}</sup>}
            </button>
          ))}
        </nav>
        <div className="header-tools">
          <button className="support-button-header" onClick={() => setSupportOpen(true)} title={t.supportBtn}>
            <HeartIcon />
        ...
        <main id="lab">
            ...
            <div className="control-panel">
              <div className="field-block base-field">
                <div className="field-label-wrap">
                  ...
                  <button className="small-tool-btn" onClick={pasteColorFromClipboard} title={t.paste}><PasteIcon /><span>{t.paste}</span></button>
                </div>
              ...
              <div className="field-block harmony-field">
                ...
                <div className="harmony-tabs">
                  {HARMONIES.map(item => (
                    <button key={item} className={harmony === item ? "active" : ""} onClick={() => handleHarmonyChange(item)}>
                      {t[item]}
                    </button>
                  ))}
                </div>
              </div>
              <div className="action-row-deck">
                <button className="generate-button" onClick={regenerate}>
                  <SparkIcon />
                  <span>{t.generate}</span>
                  <kbd>{t.shortcut}</kbd>
                </button>
                {history.length > 0 && (
                  <button className="undo-button" onClick={undo} title={t.undo}>
                    <UndoIcon />
                    <span>{t.undo}</span>
                  </button>
                )}
          ...
          <section className="palette-section">
            <div className="section-head">
              <div>
                <span className="section-kicker">{t.paletteLabel} · {t.paletteCount}</span>
                <p>{t.lockHint}</p>
              </div>
              <div className="head-tools-group">
                <button className="tool-btn" onClick={copyAllColors} title={t.copyAll}><CopyIcon /><span>{t.copyAll}</span></button>
                <button className="tool-btn" onClick={sharePalette} title={t.share}><ShareIcon /><span>{t.share}</span></button>
                <div className="format-switch" dir="ltr">
            ...
            <div className="palette-actions">
              <button onClick={savePalette}><SaveIcon />{t.save}</button>
              <button className="primary" onClick={() => setExportOpen(true)}><DownloadIcon />{t.export}</button>
            </div>
          </section>

          <PreviewCard colors={rawColors} t={t} onAction={() => { regenerate(); document.getElementById("lab")?.scrollIntoView({ behavior: "smooth" }); }} />
        </main>
      )}
      ...
      {page === "saved" && (
        <main className="page">
          <header className="page-head">
            <span className="section-kicker">{t.archiveKicker}</span>
            <h2>{t.savedTitle}</h2>
            <p>{t.savedSub}</p>
          </header>
          <div className="saved-list">
            {saved.length === 0 ? (
              <div className="empty-state">
                <p>{t.savedEmpty}</p>
                <button className="empty-cta" onClick={() => openPage("lab")}>
                  {t.goLab}<ArrowIcon />
                </button>
              </div>
            ) : (
                  ...
                  <div className="saved-info">
                    {editingSavedId === item.id ? (
                      <div className="rename-inline">
                        ...
                        <button className="rename-save" onClick={() => saveRenameSaved(item.id)}><CheckIcon /></button>
                      </div>
                    ) : (
                      <b>
                        {item.name}
                        <button className="inline-edit-btn" onClick={() => startRenameSaved(item)} title={t.rename}><EditIcon /></button>
                      </b>
                    )}
                    <small>{new Date(item.createdAt).toLocaleDateString(language === "ar" ? "ar" : "en")}</small>
                  </div>
                  <div className="saved-actions">
                    <button onClick={() => loadSaved(item)}>{t.load}</button>
                    {confirmDeleteId === item.id ? (
                      <button className="delete-confirm-btn" onClick={() => removeSaved(item.id)}>
                        {t.confirmDelete || "Confirm?"}
                      </button>
                    ) : (
                      <button className="delete" onClick={() => setConfirmDeleteId(item.id)} aria-label={t.delete} title={t.delete}>
                        <TrashIcon />
                      </button>
                    )}
                  </div>
      ...
      <nav className="bottom-nav" aria-label={t.navMobile || "Mobile Navigation"}>
        <button className={page === "lab" ? "active" : ""} onClick={() => openPage("lab")}>
          <LabIcon />
          <span>{t.navLab}</span>
        </button>
        <button className={page === "saved" ? "active" : ""} onClick={() => openPage("saved")}>
          <div className="nav-icon-wrap">
            <SaveIcon />
            {saved.length > 0 && <sup>{saved.length}</sup>}
          </div>
          <span>{t.navSaved}</span>
        </button>
        <button className={page === "about" ? "active" : ""} onClick={() => openPage("about")}>
          <InfoIcon />
          <span>{t.navAbout}</span>
        </button>
        <button className={page === "pricing" ? "active" : ""} onClick={() => openPage("pricing")}>
          <PriceTagIcon />
      ...
      {exportOpen && (
        <div className="modal-backdrop" onMouseDown={e => { if (e.currentTarget === e.target) setExportOpen(false); }}>
          <section className="modal" role="dialog" aria-modal="true">
            <button className="modal-close" onClick={() => setExportOpen(false)} aria-label={t.close}>×</button>
            <span className="section-kicker">{t.exportKicker}</span>
            <h2>{t.exportTitle}</h2>
            <p className="modal-sub">{t.exportSub}</p>
            <div className="export-tabs">
              {(["css", "tailwind", "json"] as const).map(item => (
                <button key={item} className={exportType === item ? "active" : ""} onClick={() => setExportType(item)}>
                  {t[item]}
                </button>
              ))}
            </div>
